...
const EligibleBidderList = ({ tenderId, onClose }:any) => {
    ...
    useEffect(() => {
        const viewEligibleBidders = async (tenderId:any) => {
            try {
              if (contract) {
                    const result: Bidder[] = await contract.viewEligibleBidders(tenderId);
                    setBidders(result)
                    console.log("result:",result);  // Handle the result as needed
              }
          } catch (error) {
              console.error("Error calling viewEligibleBidders:", error);
          }    
          };
        contract && viewEligibleBidders(tenderId);
...
export default EligibleBidderList